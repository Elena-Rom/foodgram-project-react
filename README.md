...
Скопировать на сервер файлы docker-compose.yaml и nginx.conf:
```
...
SSH_KEY= # SSH-key(private) компьютера, с которого будет происходить подключение к удалённому серверу
...
* сборка и доставка докер-образа для контейнеров backend и frontend на Docker Hub
...
```
### Внутри контейнера backend заустить команды:
1. Перейти в контейнер docker exec -it <CONTAINER ID> bash
